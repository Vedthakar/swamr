...
function printResumeSummary(projectDir: string, state: SwamrState) {
  const counts = {
...
function requeueRecoverableTasks(state: SwamrState): number {
  let requeued = 0;
  for (const task of state.tasks) {
...
export async function continueBuild(
  targetDir: string,
  options: { model?: string; trust?: boolean } = {}
) {
  const projectDir = path.resolve(targetDir);
  const config = loadConfig(projectDir);
