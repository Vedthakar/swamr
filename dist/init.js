...
export function init(targetDir) {
    ...
    if (fs.existsSync(path.join(agencyDir, ".git"))) {
        ...
        if (pulled === null)
            warn("Could not update, using existing");
    ...
    if (!fs.existsSync(configPath)) {
        writeFileDeep(configPath, JSON.stringify({
            version: "1.0.0",
    ...
    if (!fs.existsSync(swamrGitignore)) {
        writeFileDeep(swamrGitignore, `state.json
evidence/
scripts/
logs/
blockers/
...
`);
    ...
    if (commandExists("npx")) {
        ...
        if (pwVersion) {
            ...
            console.log("  (Needed for browser automation — Supabase setup, visual testing, etc.)");
        }
    }
    else {
...
`;
    if (fs.existsSync(projectGitignore)) {
        ...
        writeFileDeep(projectGitignore, `node_modules/
.env
.env.local
.env.*.local
${swamrIgnoreBlock}`);
    ...
    console.log();
    console.log(`    ${bold("@swamr-orchestrator Build me [describe your app]")}`);
    console.log();
    console.log(`  Or for more control:`);
    console.log();
    console.log(`    ${bold("@swamr-orchestrator Plan a [type] app with [features]")}`);
    console.log(`    ${bold("@swamr-planner Show me the task breakdown")}`);
    console.log(`    ${bold("@swamr-orchestrator Execute the plan")}`);
    console.log();
    console.log(`  To continue an interrupted build without re-planning:`);
    console.log();
    console.log(`    ${bold("swamr continue --dir <project-dir>")}`);
    console.log();
    console.log(`  The agents will automatically:`);
    console.log(`    • Initialize an Obsidian vault as the project's second brain`);
    ...
    console.log();
    console.log(`  ${bold("IMPORTANT: Use ∞ Agent mode in Cursor (not Chat mode)")}`);
    console.log(`  ${bold("Open swamr/brain/ as an Obsidian vault to watch progress live!")}`);
    console.log();
}
