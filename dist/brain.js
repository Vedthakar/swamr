...
export function initBrain(projectDir) {
    ...
    writeFileDeep(path.join(brain, "index.md"), `# Swamr Brain — Map of Content
...
\`\`\`
`);
    writeFileDeep(path.join(brain, "01-planning/domains/_index.md"), `# Planning Domains
...
| — | — | — |
`);
    writeFileDeep(path.join(brain, "00-project/overview.md"), `# Project Overview
...
- [TBD]
`);
    writeFileDeep(path.join(brain, "00-project/tech-stack.md"), `# Tech Stack
...
| Hosting | TBD | — |
`);
    writeFileDeep(path.join(brain, "00-project/architecture.md"), `# Architecture
...
`);
    writeFileDeep(path.join(brain, "00-project/glossary.md"), `# Glossary
...
| — | — |
`);
    writeFileDeep(path.join(brain, "01-planning/task-tree.md"), `# Task Tree
...
[TBD]
`);
    writeFileDeep(path.join(brain, "01-planning/requirements.md"), `# Requirements
...
- [TBD]
`);
    writeFileDeep(path.join(brain, "01-planning/risk-register.md"), `# Risk Register

| Risk | Impact | Likelihood | Mitigation |
|------|--------|-----------|-----------|
| — | — | — | — |
`);
    writeFileDeep(path.join(brain, "03-build/phase-log.md"), `# Build Phase Log
...
`);
    // Templates
    writeFileDeep(path.join(brain, "templates/task-output.md"), `---
task_id: [ID]
agent: [agent-slug]
phase: [phase number]
status: completed
...
- [Anything unresolved]
`);
    writeFileDeep(path.join(brain, "templates/decision-record.md"), `---
id: ADR-[NNN]
...
`);
    writeFileDeep(path.join(brain, "templates/phase-summary.md"), `---
phase: [number]
name: [phase name]
completed: [ISO date]
...
`);
    // Obsidian app config
    writeFileDeep(path.join(brain, ".obsidian/app.json"), JSON.stringify({
        showLineNumber: true,
